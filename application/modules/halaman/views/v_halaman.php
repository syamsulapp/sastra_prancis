<div class="row">
	<div class="col-lg-12">
		<form action="<?php echo site_url(); ?>halaman/delhalamanmassal" name="form_hapus_tulisan_massal" id="form-hapus-tulisan-massal" method="post">
			<ul class="uk-tab">
				<li class="uk-active" aria-expanded="true"><a href="#">Semua (<?php if (@$data['total_rows']) {
																					echo $data['total_rows'];
																				} else {
																					echo '0';
																				} ?>)</a></li>
				<li class=""><a href="<?php echo site_url() ?>halaman/sampah">Sampah (<?php if (@$data['total_rows_2'][0]['total']) {
																							echo $data['total_rows_2'][0]['total'];
																						} else {
																							echo '0';
																						} ?>)</a></li>
			</ul>
			...
			<div class="block" style="margin-bottom:30px;">
				<div class="form-group">
					...
					<div style="float:right" class="row col-md-4">
						<div class="input-group">
							<input value="<?php if (@$_GET['cari']) echo $_GET['cari']; ?>" onblur="cari(this.value)" type="text" class="form-control square-btn-adjust" title="Cari halaman">
							<div class="input-group-btn">
								<a class="btn btn-default"><i class="fa fa-search"></i></a>
								<a href="<?php echo site_url() . 'halaman'; ?>" class="btn btn-default"><i class="fa fa-refresh"></i></a>
							</div>
						</div>
					</div>
					<br />
				...
				<table class="uk-table">
					<thead>
						<tr>
							...
							<th colspan="4" width="5%" class="md-bg-grey-100">Aksi</th>
						</tr>
					</thead>
					<tbody id="tbody-tulisan">
						...
						?>
								<tr>
									...
									<td><?php echo $no ?></td>
									<td><img src="<?php if ($row['gambar_andalan'] != '') {
														echo base_url('assets/img/img_andalan/thumb') . '/' . $row['gambar_andalan'];
													} ?>" style="width:30px;height:30px" alt="   "> <?php echo $row['judul_id'] ?> | <?php echo $row['judul_eng'] ?> | <?php echo $row['judul_ae'] ?> <?php if ($row['status_tulisan'] == 'draft') {
																																																																												echo '(<i>draft</i>)';
																																																																											} ?></td>
									<td><?php echo $row['tgl_tulisan'] ?></td>
									<td><?php echo $row['view'] ?></td>
									<td>
										<a target="blank" href="<?php echo site_url() . 'home/halaman/' . $row['id_tulisan'] . '/' . preg_replace("/[\/\!@#$%^&*()=+{},]/", "_", str_replace(' ', '-', strtolower($row['judul_id']))) . '/preview'; ?>" title="preview" class="text-primary">
									...
									<?php if ($this->session->userdata('id_pengguna') == $row['penulis'] || $this->session->userdata('level') == 'administrator' || $this->session->userdata('level') == 'editor') { ?>
										<td id="tmp_smb_<?php echo $row['id_tulisan']; ?>">
											<?php if ($row['status_tulisan'] == 'terbit') { ?>
												<a href="#" title="Sembunyikan" onclick="set_sembunyikan('<?php echo $row['id_tulisan'] ?>')" class="text-warning">
													<i class="fa fa-eye"></i>
												</a>
											<?php } else { ?>
												<a href="#" title="Tampilkan" onclick="set_tampilkan('<?php echo $row['id_tulisan'] ?>')" class="text-warning">
													<i class="fa fa-eye-slash"></i>
												</a>
											<?php } ?>
										</td>
										<td><a href="<?php echo site_url() . 'halaman/sunting?id=' . $row['id_tulisan']; ?>" title="Ubah" class="text-success">
												<i class="fa fa-pencil"></i></a>
										</td>
										<td><a href="#" title="Hapus" onclick="set_delete({
								id:'<?php echo $row['id_tulisan'] ?>',
								nm:'<?php echo $row['judul_id'] ?>'
								,act:'<?php echo site_url(); ?>halaman/delhalaman'
			...
			<div class="block" style="margin-top:10px;">
				<div class="form-group">
					...
					<div style="float:right">
						<i>
							<gedank id="jml-list-bawah"><?php if (@$data['total_rows']) echo $data['total_rows']; ?></gedank> tulisan
						</i>
					</div>
				</div>
			</div>
...
<div class="md-fab-wrapper">
	<a href="<?php echo site_url(); ?>halaman/baru" class="md-fab md-fab-accent">
...
<script>
	...
	function set_sembunyikan(id) {
		$.ajax({
			'url': '<?php echo site_url("halaman/sembunyikan/'+id+'") ?>',
			'dataType': 'json',
	...
	function set_tampilkan(id) {
		$.ajax({
			'url': '<?php echo site_url("halaman/tampilkan/'+id+'") ?>',
			'dataType': 'json',
			success: function(data) {
				console.log(data);
				if (data.psn != null) {
	...
	function cari(val) {
		window.location.assign("<?php echo site_url() ?>halaman?cari=" + val)
	}
</script>
