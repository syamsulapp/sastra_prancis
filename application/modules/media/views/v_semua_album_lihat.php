...
<div class="row">
	<h2>
		<?php if(isset($album['tulisan'][0]['judul'])){echo$album['tulisan'][0]['judul'];}?>
		...
		<a id="btn-tambah-album-baru" class="btn btn-default square-btn-adjust" title="Tambah album baru" data-toggle="modal" data-target=".bs-album"><li class="fa fa-plus"></li> Tambah gambar</a>
		<?php }?>
	</h2>
</div>
<div class="row">
	...
	$.each(data.gambar, function(i,n){
		thumbnail+='<div class="col-lg-2 col-md-3 col-xs-4 thumb text-center" id="hapus-row-album-'+n.replace('.','')+'" onmouseover="row_actions_over(\''+n.replace('.','')+'\')" onmouseout="row_actions_blur(\''+n.replace('.','')+'\')">'
	        ...
	        if(penulis==id_penulis||admin=='administrator'||admin=='editor'){
	            ...
	            +'<p class="row-actions" id="row-actions-'+n.replace('.','')+'">'
		        	+'<span class="hapus"><a title="Hapus '+n+'" data-toggle="modal" data-target=".bs-hapus-img" onclick="set_del_({id:\''+n+'|'+id+'\',nm:\''+n+' dari album '+id+'\',act:\''+url+'album/delgambaralbum\',element:\'hapus-row-album-'+n.replace('.','')+'\'})">Hapus</a></span>'
		        +'</p>';
	    	}
        thumbnail+='</div>';
	});
