<div class="row">
	<div class="col-lg-12">
		<form action="<?php echo site_url();?>tulisan/deltulisanmassal" name="form_hapus_tulisan_massal" id="form-hapus-tulisan-massal" method="post">
		<ul class="uk-tab">
            <li class="uk-active" aria-expanded="true"><a href="#">Semua (<?php if(@$data['total_rows']){echo$data['total_rows'];}else{echo'0';}?>)</a></li>
            <li class=""><a href="<?php echo site_url()?>tulisan/sampah">Sampah (<?php if(@$data['total_rows_2'][0]['total']){echo$data['total_rows_2'][0]['total'];}else{echo'0';}?>)</a></li>
        </ul>
        <br>
				...
				<div style="float:right" class="row col-md-4">
					<div class="input-group">
					  ...
					  <div class="input-group-btn">
					    ...
					    <a href="<?php echo site_url().'tulisan';?>" class="btn btn-default"><i class="fa fa-refresh"></i></a>
					  </div>
					</div>
				</div>
				<br>
			...
			<table class="uk-table">
				<thead>
					<tr>
						...
						<th colspan="4" width="5%" class="text-center md-bg-grey-100">Aksi</th>
					</tr>
				</thead>
				...
				?>
					<tr>
						...
						<td><?php //if($row['slug']!='quote'){?>
							<a target="blank" href="<?php echo site_url().'home/'.$row['slug'].'/'.$row['id_tulisan'].'/'.preg_replace("/[\/\!@#$%^&*()=+{},]/", "_",str_replace(' ','-',strtolower($row['judul_id']))).'/preview';?>" title="preview" class="text-primary">
						...
						<td><a href="#" title="Hapus" onclick="set_delete({
								...
								,act:'<?php echo site_url();?>tulisan/deltulisan'
				...
				<tfoot>
					<tr>
						...
						<th colspan="4" class="text-center md-bg-grey-100">Aksi</th>
					</tr>
				</tfoot>
			</table>
			<!--tabel tulisan end-->
...
function set_format(id){
	window.location.href = "<?php echo site_url('tulisan?id=')?>"+id;
}
...
function set_sembunyikan(id){
	$.ajax({
        'url': '<?php echo site_url("tulisan/sembunyikan/'+id+'")?>',
        'dataType': 'json',
...
function set_tampilkan(id){
	$.ajax({
        'url': '<?php echo site_url("tulisan/tampilkan/'+id+'")?>',
        'dataType': 'json',
        success: function(data){
        	console.log(data);
            if(data.psn!=null){
...
function cari(val){
	window.location.assign("<?php echo site_url()?>tulisan?cari="+val)
}
</script>
