<div class="row">
	<div class="col-lg-12">
		<form action="<?php echo site_url();?>komentar/delkomentarmassal" name="form_hapus_tulisan_massal" id="form-hapus-tulisan-massal" method="post">
		<ul class="uk-tab">
				...
				<div style="float:right" class="row col-md-4">
					<div class="input-group">
					  ...
					  <div class="input-group-btn">
					    ...
					    <a href="<?php echo site_url().'komentar';?>" class="btn btn-default"><i class="fa fa-refresh"></i></a>
					  </div>
					</div>
				</div>
				<br />
		...
		<div class="table-responsive">
			...
			<table class="uk-table">
				<thead>
					<tr>
						<th width="3%" class="md-bg-grey-100">
							<?php if($this->session->userdata('level')!='user'){?>
				...
				?>
					<tr>
						...
						<td id="tmp_smb_<?php echo$row['id_komentar'];?>">
							<?php if($row['status_komentar_']=='terbit'){?>
							<a href="#" title="Sembunyikan" onclick="set_sembunyikan('<?php echo$row['id_komentar']?>')" class="text-warning">
								<i class="fa fa-eye"></i>
							</a>
							<?php }else{ ?>
							<a href="#" title="Tampilkan" onclick="set_tampilkan('<?php echo$row['id_komentar']?>')" class="text-warning">
						...
						<td><a href="#" title="Hapus" onclick="set_delete({
								...
								,act:'<?php echo site_url();?>komentar/delkomentar'
...
function set_sembunyikan(id){
	$.ajax({
        'url': '<?php echo site_url("komentar/sembunyikan/'+id+'")?>',
        'dataType': 'json',
...
function set_tampilkan(id){
	$.ajax({
        'url': '<?php echo site_url("komentar/tampilkan/'+id+'")?>',
        'dataType': 'json',
        success: function(data){
        	console.log(data);
            if(data.psn!=null){
...
function cari(val){
	window.location.assign("<?php echo site_url()?>komentar?cari="+val)
}
</script>
