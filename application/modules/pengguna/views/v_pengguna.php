...
<div class="row">
	<div class="col-lg-12">
		<form action="<?php echo site_url();?>pengguna/delpenggunamassal" name="form_hapus_pengguna_massal" id="form-hapus-pengguna-massal" method="post">
		<ul class="uk-tab">
			<?php if($this->uri->segment(2)==''or$this->uri->segment(2)=='pengguna'){?>
				<li class="uk-active"><a href="#">Semua (<?php if(@$data['total_rows_1'][0]['total']){echo$data['total_rows_1'][0]['total'];}else{echo'0';}?>)</a></li>
			<?php }else{ ?>
				<li class=""><a href="<?php echo site_url(); ?>pengguna/pengguna">Semua (<?php if(@$data['total_rows_1'][0]['total']){echo$data['total_rows_1'][0]['total'];}else{echo'0';}?>)</a></li>
			<?php }?>

			<?php if($this->uri->segment(2)=='administrator'){?>
				<li class="uk-active"><a href="#">Administrator (<?php if(@$data['total_rows_2'][0]['total']){echo$data['total_rows_2'][0]['total'];}else{echo'0';}?>)</a></li>
			<?php }else{ ?>
				<li class=""><a href="<?php echo site_url(); ?>pengguna/administrator">Administrator (<?php if(@$data['total_rows_2'][0]['total']){echo$data['total_rows_2'][0]['total'];}else{echo'0';}?>)</a></li>
			<?php }?>
			<?php if($this->uri->segment(2)=='editor'){?>
				<li class="uk-active"><a href="#">Editor (<?php if(@$data['total_rows_3'][0]['total']){echo$data['total_rows_3'][0]['total'];}else{echo'0';}?>)</a></li>
			<?php }else{ ?>
				<li class=""><a href="<?php echo site_url(); ?>pengguna/editor">Editor (<?php if(@$data['total_rows_3'][0]['total']){echo$data['total_rows_3'][0]['total'];}else{echo'0';}?>)</a></li>
			<?php }?>
			<?php if($this->uri->segment(2)=='user'){?>
				<li class="uk-active"><a href="#">User (<?php if(@$data['total_rows_4'][0]['total']){echo$data['total_rows_4'][0]['total'];}else{echo'0';}?>)</a></li>
			<?php }else{ ?>
				<li class=""><a href="<?php echo site_url(); ?>pengguna/user">User (<?php if(@$data['total_rows_4'][0]['total']){echo$data['total_rows_4'][0]['total'];}else{echo'0';}?>)</a></li>
			<?php }?>
			...
			<?php }else{ ?>
				<li class=""><a href="<?php echo site_url(); ?>pengguna/block">Block (<?php if(@$data['total_rows_5'][0]['total']){echo$data['total_rows_5'][0]['total'];}else{echo'0';}?>)</a></li>
			<?php }?> 
		</ul>
        <br>
			...
			<div class="form-group">
				<div style="float:left" class="row col-md-4">
					<div class="input-group">
						...
						<span class="btn btn-default square-btn-adjust input-group-addon" title="Terapkan" onclick="submit('#form-hapus-pengguna-massal');">Terapkan</span>
					</div>
				</div>
				<div style="float:right" class="row col-md-4">
					<div class="input-group">
					  <input value="<?php if(@$_GET['cari'])echo$_GET['cari'];?>" onblur="cari(this.value)" type="text" class="form-control square-btn-adjust" title="Cari pengguna">
					  <div class="input-group-btn">
					  	<a class="btn btn-default"><i class="fa fa-search"></i></a>
					    <a href="<?php echo site_url().'pengguna';?>" class="btn btn-default"><i class="fa fa-refresh"></i></a>
					  </div>
					</div>
				</div>
				<br />
			...
			<table class="uk-table">
				<thead>
					<tr>
						...
						<th class="md-bg-grey-100">Username</th>
						<th width="20%" class="md-bg-grey-100">Nama</th>
						<th width="10%" class="md-bg-grey-100">Email</th>
						<th width="10%" class="md-bg-grey-100">Web</th>
						<th width="15%" class="md-bg-grey-100">Tgl Bergabung</th>
						<th colspan="3" width="5%" class="md-bg-grey-100">Aksi</th>
					</tr>
				</thead>
				<tbody id="tbody-pengguna">
					<?php if(@$data['pengguna']&&$data['error']==0){
					$no=$data['no']+1;
					foreach($data['pengguna'] as $row){
					?>
					<tr>
						<td>
							<input name="check_list[<?php echo$row['id_pengguna'];?>]" value="<?php echo$row['id_pengguna'];?>" id="check-list-<?php echo$no;?>" class="check-list" type="checkbox">
						</td>
						<td><img src="<?php if($row['foto']!=''){echo base_url('assets/img/img_andalan/thumb').'/'.$row['foto'];}?>" style="width:30px;height:30px" alt=""> <?php echo$row['username']?></td>
						...
						<td id="tmp_smb_<?php echo$row['id_pengguna'];?>">
							<?php if($row['status_pengguna']=='aktif'){?>
							<a href="#" title="Block" onclick="set_sembunyikan('<?php echo$row['id_pengguna']?>')" class="text-warning">
								<i class="fa fa-eye"></i>
							</a>
							<?php }else{ ?>
							<a href="#" title="Aktifkan" onclick="set_tampilkan('<?php echo$row['id_pengguna']?>')" class="text-warning">
								<i class="fa fa-eye-slash"></i>
							</a>
							<?php }?>
						</td>
						<td><a href="<?php echo site_url().'pengguna/sunting?id='.$row['id_pengguna'];?>" title="Ubah" class="text-success">
							<i class="fa fa-pencil"></i></a>
						</td>
						<td><a href="#" title="Hapus" onclick="set_delete({
								id:'<?php echo$row['id_pengguna']?>',
								nm:'<?php echo$row['username']?>'
								,act:'<?php echo site_url();?>pengguna/delpengguna'
				...
				<tfoot>
					<tr>
						...
						<th class="md-bg-grey-100">Web</th>
						<th class="md-bg-grey-100">Tgl Bergabung</th>
		...
		<div class="block" style="margin-top:10px;">
			<div class="form-group">
				...
				<div style="float:right">
					<i><gedank id="jml-list-bawah"><?php if(@$data['total_rows'])echo$data['total_rows'];?></gedank> pengguna</i>
				</div>
			</div>
		</div>
		</form>
...
<div class="md-fab-wrapper">
    <a href="<?php echo site_url('pengguna/baru');?>" id="btn-tambah-testimoni-baru" class="md-fab md-fab-accent">
				...
				<div class="col-lg-12">
					<form id="form_action_del" action="" method="post">
						<div class="modal-header">
							...
							<h3 id="modal_title" class="modal-title">Hapus Pengguna</h3>
						</div>
...
function set_sembunyikan(id){
	$.ajax({
        'url': '<?php echo site_url("pengguna/sembunyikan/'+id+'")?>',
        'dataType': 'json',
        success: function(data){
        	console.log(data);
            if(data.psn!=null){
...
function set_tampilkan(id){
	$.ajax({
        'url': '<?php echo site_url("pengguna/tampilkan/'+id+'")?>',
        'dataType': 'json',
        success: function(data){
        	console.log(data);
            if(data.psn!=null){
...
function cari(val){
	window.location.assign("<?php echo site_url()?>pengguna/<?php echo $this->uri->segment(2)?>?cari="+val)
}
</script>
